...
function SubMenu({mainSubItem,mainSubDesc,secondSubItem,secondSubDesc,lastSubItem,lastSubDesc,base_color,bg_img,isOwnerActive,isFreelanceActive}) {
  const  {user}= useUser()
  return (
    <div className="z-50 bg-white absolute left-0 top-[48px] whitespace-break-spaces w-full min-h-[380px] shadow-md cursor-default">
      <div className="flex flex-wrap w-full h-auto py-[30px] px-10  gap-8 bg-white">
        ...
        <div className="flex flex-col gap-5 font-DanaBold z-20 text-black_base">
          <Link to={`${user  && isOwnerActive ? "/owner/new-project" : user &&  isFreelanceActive ? "/freelancer/projectLists" :""}`}
        ...
        <div className="flex flex-col gap-5 text-black_base z-20">
          <Link  to={`${user  && isOwnerActive ? "/owner" : user &&  isFreelanceActive ? "/freelancer" :""}`} className="w-[300px] px-4 py-2 rounded-lg tr-300 hover:bg-hover">
